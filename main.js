...
form.addEventListener("submit", async (e) => {
  ...
  const user = data[0];
  if (user && user.Approval === true && user.Devices === deviceId) {
    window.location.href = "main.html";
  } else {
    await supabase.from("User").upsert({ Email: email, Devices: deviceId, Approval: false });
    sendRequestEmail(email, deviceId);
...
function sendRequestEmail(email, deviceId) {
  // ⚠️ Tùy chọn: Tích hợp với EmailJS hoặc webhook server tại đây
